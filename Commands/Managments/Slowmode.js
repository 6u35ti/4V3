...
module.exports = {
    ...
    options:[
      {
        ...
        required:true,
        choices:[
          { name:"0 off",        value:"0 seconds"  },
          { name:"5 seconds",    value:"5 seconds"  },
          { name:"10 seconds",   value:"10 seconds" },
          { name:"20 seconds",   value:"20 seconds" },
  ...
  if (!channel) channel = interaction.channel;
        ...
        return interaction.editReply({content:`You Dont have \`MANAGE_CHANNELS\` Permission`});

        if (!interaction.guild.me.permissions.has(`MANAGE_CHANNELS`)) 
        return interaction.editReply({content:`I Dont have \`MANAGE_CHANNELS\` permission`});
        
        choice = choice.replace("0 seconds",     "0")
        ...
        await interaction.reply({ 
            embeds: [
                new MessageEmbed()
                .setColor("#303236")
                .setDescription(`✅ | Slowmode has been \`${choice}s\`activated`)
            ]
        })
