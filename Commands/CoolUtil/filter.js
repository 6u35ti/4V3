    ...
    run: async (client, interaction, args, prefix) => {
        try {
            ...
            const Menu = new MessageSelectMenu()
                ...
                .setPlaceholder("Pick Filters to enable/disable")
            ...
            collector.on("collect", i => {
                ...
                i.reply({
                    ...
                    embeds: [
                        ...
                        .setDescription(Object.keys(queue.effects).filter(o => o != "bassboost" && o != "speed").map(option => `> **\`${option.charAt(0).toUpperCase()}${option.slice(1)}\`** - ${queue.effects[option] ? `✅ Enabled` : `:no_entry_sign: Disabled`}`).join("\n\n"))
                    ]
                })
...
run: async (client, interaction, args, prefix) => {
        try {
            ...
            const queue = client.queues.get(interaction.guild.id);
            if(!queue) return interaction.reply({ ephemeral: true, content:`<:oddno:968555009908293652> **I'm nothing playing right now.**`}).catch(() => null);
            ...
            console.error(e);
            interaction.reply({ ephemeral: true, content:`<:oddno:968555009908293652> Something went wrong: \`\`\`${e.interaction || e}`.substring(0, 1950) + `\`\`\``}).catch(() => null);
*/